...
// 团队各 Agent 的完整工作区。卷宗页只读展示其中的文本文件。
const TEAM_WORKSPACES = [
  ['main',      path.join(OPENCLAW_ROOT, 'workspace')],
  ['content',   path.join(OPENCLAW_ROOT, 'workspace-content')],
  ['tech',      path.join(OPENCLAW_ROOT, 'workspace-tech')],
  ['intel',     path.join(OPENCLAW_ROOT, 'workspace-intel')],
  ['assistant', path.join(OPENCLAW_ROOT, 'workspace-assistant')],
];
...
function getJobUuidMap() {
  ...
  const raw = jobsFile ? safeReadJson(jobsFile, []) : [];
  const jobs = Array.isArray(raw)
    ? raw
    : (Array.isArray(raw.list) ? raw.list : (raw.jobs ? Object.entries(raw.jobs).map(([id, v]) => ({ id, ...v })) : []));
  const nameKeywords = {
...
function readGatewayRuns(shortId, limit) {
  ...
  const uuid = uuidMap[shortId] || shortId;
  if (!uuid) return [];
  const runsFile = [
    path.join(GW_CRON_RUNS_DIR, `${uuid}.jsonl`),
    path.join(GW_CRON_RUNS_DIR, `${uuid}.jsonl.migrated`),
  ].find((file) => fs.existsSync(file));
  if (!runsFile) return [];
  try {
